...
device = None
BATCH_SIZE = None
batch_test_flag = None


def init(args_source):
    global args
    global Ks
    global device
    global BATCH_SIZE
    global batch_test_flag
...
def ranklist_wo_auc(user_pos_test, test_items, rating, Ks):
    item_score = {}
    for i in test_items:
...
def ranklist_w_auc(user_pos_test, test_items, rating, Ks):
    item_score = {}
    for i in test_items:
        item_score[i] = rating[i]
...
def test_one_user(x):
    ...
    if args.auc_cal == True:
        r, auc = ranklist_w_auc(user_pos_test, test_items, rating, Ks)
    else:
        r, auc = ranklist_wo_auc(user_pos_test, test_items, rating, Ks)
...
def test(model, user_dict, n_params):
    ...
    for u_batch_id in range(n_user_batchs): #取得一個batch user embedding
        ...
        if batch_test_flag:
            ...
            i_count = 0
            for i_batch_id in range(n_item_batchs): #取得一個batch item embedding
